...
if(isset($_POST['submit'])){
    ...
    $email_check = "SELECT * FROM kopy_user WHERE email = '$email'";
    $res = mysqli_query($conn, $email_check);
    

    $insert =  "INSERT INTO kopy_user(name,username,email,password)VALUES
...
if(mysqli_num_rows($res) > 0){
  ...
  <strong> '.$email .'</strong> already Exist
        ...
        $success=mysqli_query($conn,$insert) or die();
        if ($success)
    {  
          ...
          <strong>email'.$email .'</strong>Not matched
          </button>
        </div>';
         }
        if ($success) {
